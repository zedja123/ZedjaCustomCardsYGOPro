...
function c270000105.tdop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if tc then
		if tc:IsSetCard(0xf11) and tc:IsType(TYPE_SPELL) and tc:IsControler(tp) and Duel.SelectYesNo(tp,aux.Stringid(270000105,3)) then
...
function c270000105.lvop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c270000105.filter,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
	for tc in aux.Next(g) do
		c270000105.change_level(tc)
		c270000105.change_rank(tc)
	end
end

function c270000105.filter(c)
	return c:IsFaceup() and (c:IsLevelAbove(1) or c:GetRank()>0)
end

function c270000105.change_level(c)
	local e1=Effect.CreateEffect(c)
...
function c270000105.change_rank(c)
	local e2=Effect.CreateEffect(c)
